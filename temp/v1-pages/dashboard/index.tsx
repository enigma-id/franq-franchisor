...
const DashboardPage: React.FC = () => {
  ...
  const topSalesOutlets: { name: string; quantity: number; omset: number }[] =
    Array.isArray(posReport?.top_sales_outlet)
      ? posReport.top_sales_outlet
      : [];
  ...
  const maxSalesItemOmset = topSalesItems.reduce(
    (m, i) => Math.max(m, i.omset),
    0,
  );
  const maxSalesOutletOmset = topSalesOutlets.reduce(
    (m, i) => Math.max(m, i.omset),
    0,
  );

...
const TableFilter: React.FC<TableFilterProps> = ({ table }) => {
  ...
  // Sync state when current.outlet_id changes
  useEffect(() => {
    if (current.outlet_id && getResult?.data?.data) {
      const outlet = getResult.data.data as any[];
      const found = outlet.find((c) => c.id === current.outlet_id);
      if (found) {
        setOutlet(found);
      }
    ...
    <div className="flex flex-row items-center gap-3 w-full shrink-0">
      <div className="w-40 md:w-56">
        <RemoteSelect
          ...
          getLabel={(item: any) => (item ? item.alias : "")}
          renderItem={(item: any) => item?.alias}

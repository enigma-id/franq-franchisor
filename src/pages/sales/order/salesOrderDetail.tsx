...
export default function SalesOrderDetailPage() {
  ...
  const totalQty =
    order.items?.reduce((sum, item) => sum + (item.quantity_ordered || 0), 0) ||
    0;
              ...
              <dl className='space-y-1'>
                ...
                <div className='info-row flex-col items-start gap-1'>
                  ...
                  <dd className='info-value text-left w-full wrap-break-words mt-0.5'>
                    {[order.recipient_address, formatRegion(order?.region)]
                      .filter(Boolean)
                      .join(", ")}
                  </dd>
                </div>
          ...
          <div className='card-info card-animate p-6'>
            ...
            <dl className='space-y-1'>
              <div className='info-row'>
                ...
                <dd className='info-value'>{order.payment_method?.name}</dd>
              </div>
              <div className='info-row'>
                ...
                {order.items?.length === 0 ? (
                  ...
                  order.items?.map((item: any, idx: number) => (
                    ...
                    >
                      ...
                      <td className='px-4 py-3 align-middle text-[13px] font-medium text-gray-700 text-right'>
                        {formatCurrency(
                          item.unit_nett * item.quantity_ordered || 0,
                        )}
                      </td>
                    </tr>
                  ))

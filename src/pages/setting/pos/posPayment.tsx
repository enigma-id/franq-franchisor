...
export function PosPayment() {
  ...
  const Table = useTable("setting_pos_payment", tableConfig as TableConfig<unknown>);
  ...
  const openDelete = (row: any) => {
    openModal({
      ...
      content: (
        ...
        >
          <Modal.Header>
            <div className="font-bold text-lg text-slate-900 leading-7">Hapus Metode Pembayaran</div>
          </Modal.Header>
          <Modal.Body className="text-sm font-normal text-slate-600 leading-5">
            <p>Apakah Anda yakin ingin menghapus metode pembayaran <strong>{row?.name}</strong>?</p>
          </Modal.Body>
        ...
        <Modal.Header>
          <div className="flex flex-col text-left">
            <span className="text-lg font-bold text-slate-900">
              {editingItem ? "Ubah Metode Pembayaran" : "Tambah Metode Pembayaran"}
            </span>
            <span className="text-xs text-slate-500 font-medium mt-0.5">
              {editingItem ? "Ubah detail metode pembayaran kasir." : "Buat metode pembayaran baru untuk mesin kasir POS."}
            </span>
          </div>
        </Modal.Header>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="pt-2">
              <Checkbox
                ...
                onChange={(e) => setFormData((prev) => ({ ...prev, is_nfc: e.target.checked }))}
                variant="primary"
              />
            </div>
          </form>
